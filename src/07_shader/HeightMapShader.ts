...
export class HeightMapShader extends Filter {
  constructor() {
...
#extension GL_OES_standard_derivatives : enable
  ...
  {
    vec3 c = texture2DLodEXT(tex, uv, 0.0).xyz;
  ...
  {
    return luminance(texture2D(uSampler, vTextureCoord), worldToTex(p))*0.5;
  }
  ...
  void main(void)
  {
      vec2 pixel = (vTextureCoord.xy / filterArea.xy)*2.0-1.0;

      // compute ray origin and direction
      float asp = filterArea.x / filterArea.y;
